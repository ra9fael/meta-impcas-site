...
# "save" records the current time to /boot (the writable BOOT partition,
# mounted by bootmount in the local-fs phase). Both only ever move the
# recorded time FORWARD, so a board booting into the kernel's default time
# (no RTC, NTP not yet reachable) can neither regress the clock nor
# overwrite a good timestamp with a stale one.

set -e

CLOCK_FILE=/boot/clock.save
